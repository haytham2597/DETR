...
public:
	...
	{
		MESSAGE_LOG("Step3")
		auto marked = torch::where(state_->marked_ == 1);
		auto zer = torch::zeros({ static_cast<int>(marked.size()) }, torch::kBool);
		MESSAGE_LOG_ObJ("Marked:", marked)
		MESSAGE_LOG_ObJ("Zer:", zer)
		for(int64_t n=0;n<marked.size();n++)
		{
			for (int i =0;i<marked[n].size(0);i++)
			for(int j=0;j<marked[n].size(1);j++)
				zer[n] = marked[n][i][j].item<int>() != 0;
		}

		state_->col_uncovered_.index_put_({ zer }, false);
		if(torch::cat(marked).sum().item<int>() < state_->c_.size(0))
			return step4();
		return false;
	}
	...
	{
		MESSAGE_LOG("Step4");
		auto c = torch::cat(torch::where(state_->c_ == 0)).to(torch::kInt);
		...
		while(true)
		{
			...
			auto row = vec_rowcol[0][0].item<int>();
			auto col = vec_rowcol[1][0].item<int>();
			...
			{
				state_->marked_ = state_->marked_.index_put_({ row, col }, 2);
				auto star_col = torch::argmax(torch::cat(torch::where(state_->marked_.index({ row }) == 1)))[0].item<int>();
				...
				{
					col = star_col;
					state_->row_uncovered_ = state_->row_uncovered_.index_put_({ row }, false);
					state_->col_uncovered_ = state_->col_uncovered_.index_put_({ col }, true);
					covered_c.index({ torch::indexing::Slice(), col }) = covered_c.index({ torch::indexing::Slice(), col }) * state_->row_uncovered_.to(torch::kInt);
					covered_c = covered_c.index_put_({ row }, 0);
				}
			}
		}
	}
	...
	bool step5()
	{
		...
		path = path.index_put_({ count, 0 }, state_->z0_r_);
		path = path.index_put_({ count, 1 }, state_->z0_c_);
		while(true)
		{
			auto row = torch::argmax(torch::cat(torch::where(state_->marked_.index({ torch::indexing::Slice(), path.index({count, 1}) }) == 1)));
			...
			{
				count += 1;
				path = path.index_put_({ count, 0 }, row);
				path = path.index_put_({ count, 1 }, path.index({count-1, 1}));
			}
			auto col = torch::argmax(torch::cat(torch::where(state_->marked_.index({ path.index({ count, 0 }) }) == 1))).item<int>();
			if (state_->marked_.index({ row, col }).item<int>() != 2)
				col = -1;
			count += 1;
			path = path.index_put_({ count, 0 }, path.index({ count - 1, 0 }));
			path = path.index_put_({ count, 1 }, col);
		}
		...
		state_->clear_covers();
		state_->marked_ = state_->marked_.index_put_({ torch::cat(torch::where(state_->marked_ == 2)) }, 0);
		return step3();
	}
	...
	{
		MESSAGE_LOG("Step6");
		if(anyext(torch::any(state_->row_uncovered_)) && anyext(torch::any(state_->col_uncovered_)))
		{
			torch::Tensor minVal;
			if(state_->maximize_)
			{
